...
class LogisticRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        """
        if self.include_intercept_: X = np.insert(X, 0, 1, axis=1)

        self.coefs_ = np.random.normal(0,1,X.shape[1])
        self.coefs_ /= np.sqrt(X.shape[1])

        if self.penalty_ is 'none': model = LogisticModule(self.coefs_)
        elif self.penalty_ is 'l1': model = RegularizedModule(LogisticModule(), L1(), self.lam_, self.coefs_, self.include_intercept_)
        elif self.penalty_ is 'l2': model = RegularizedModule(LogisticModule(), L2(), self.lam_, self.coefs_, self.include_intercept_)
