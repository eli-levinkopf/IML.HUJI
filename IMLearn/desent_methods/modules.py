...
class LogisticModule(BaseModule):
    ...
    def compute_jacobian(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        """
        return -(y @ X - X.T @ (np.exp(X @ self.weights) / (1 + np.exp(X @ self.weights)))) / X.shape[0]
...
class RegularizedModule(BaseModule):
    ...
    def compute_output(self, X, y, **kwargs) -> np.ndarray:
        ...
        output: ndarray of shape (1,)
            Value of function at point self.weights
        """
        return self.fidelity_module_.compute_output(X=X, y=y) + self.lam_ * self.regularization_module_.compute_output(X=X, y=y)
    ...
    def compute_jacobian(self, X, y, **kwargs) -> np.ndarray:
        ...
        if self.include_intercept_:
            return self.fidelity_module_.compute_jacobian(X=X, y=y) + self.lam_ * np.insert(self.regularization_module_.compute_jacobian(X=X, y=y), 0, 0)
        
        return self.fidelity_module_.compute_jacobian(X=X, y=y) + self.lam_ * self.regularization_module_.compute_jacobian(X=X, y=y)
    ...
    def weights(self, weights: np.ndarray) -> None:
        ...
        if self.include_intercept_: self.regularization_module_.weights = weights[1:]
        else: self.regularization_module_.weights = weights
